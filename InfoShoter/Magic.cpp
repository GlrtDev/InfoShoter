...
Magic::Magic(int level, float seedAsFrameTimeSec, int playerMagicLevel) : m_projectileLoaded(true), m_playerMagicLevel(playerMagicLevel)
{
	...
	if (m_damage > m_speed)  // IDK WHY FIRST DONT WORK 
		m_rechargeTime = (5 * (float)m_damage) / ((float)m_speed * (float)level * (float)(m_playerMagicLevel + 1)); // i have to cast level and m_plMAgicLv WHY ?? STIL DONT WORK
	else
		m_rechargeTime = 1.0f;
...
void Magic::UpdateProjectiles(sf::Time & frameTime) {
	for (auto projectile = m_projectiles.begin(); projectile != m_projectiles.end(); ++projectile) {
...
void Magic::LevelUp()
{
	m_damage *= (1 + (1.0f/5.0f * (1 - (m_type / 6.f)) ) );
